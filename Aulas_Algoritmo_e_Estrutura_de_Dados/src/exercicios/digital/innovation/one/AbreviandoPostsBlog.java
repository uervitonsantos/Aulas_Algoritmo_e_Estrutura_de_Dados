...
public class AbreviandoPostsBlog {
	...
	public static void main(String[] args) throws IOException {
		...
		List<String> alfabeto = new ArrayList<String>(Arrays.asList("abcdefghijklmnopqrstuvwxyz".split("")));
		...
		while (true) {
			...
			String frase = sc.nextLine().trim().toLowerCase().replaceAll("\n", " ").replaceAll("\t", " ");
			...
			List<String> vetPalavrasFrase = new ArrayList<String>(Arrays.asList(frase.split(" ")));
			...
			alfabeto.stream().forEach(letra -> {
				...
				for (String chave : chaves) {
					...
					qtTotalCharMelhorCenario = (qtTotalCharPorLetra - (map.get(chave) * chave.length()))
							+ (map.get(chave) * 2);
			...
			String fraseAbreviada = vetPalavrasFrase.stream().map(palavra -> {
